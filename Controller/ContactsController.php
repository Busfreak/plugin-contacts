...
class ContactsController extends BaseController {
    ...
    public function project(array $values = array(), array $errors = array())
    {
        ...
        $this->response->html($this->helper->layout->project('contacts:project/contacts', array(
            'title' => t('Contacts'),
            'values' => $values,
            'errors' => $errors,
            'project' => $project,
    ...
    public function save()
    {
        ...
        if ($valid) {
            if ($this->contactsModel->create($values) !== false) {
                ...
                return $this->response->redirect($this->helper->url->to('ContactsController', 'project', array('plugin' => 'contacts', 'project_id' => $project['id'])));
            } else {
    ...
    public function task_footer() {
        ...
        $this->response->html($this->helper->layout->app('contacts:task/contacts', array(
            'title' => t('Contacts'),
    ...
    public function edit(array $values = array(), array $errors = array())
    {
        ...
        $this->response->html($this->helper->layout->project('contacts:contact/edit', array(
...
#        }
    ...
    public function confirm()
    {
        ...
        $this->response->html($this->helper->layout->project('contacts:contact/remove', array(
            'project' => $project,
            'contact' => $contact[0],
            'contacts_id' => $this->request->getIntegerParam('contacts_id'),
...
#        $this->checkPermission($project, $filter);
        ...
        $this->response->redirect($this->helper->url->to('ContactsController', 'project', array('plugin' => 'contacts', 'project_id' => $project['id'])));
    }
    ...
    public function task() {
        ...
        $this->response->html($this->helper->layout->task('contacts:task/add', array(
            'title' => t('Contacts'),
    ...
    public function addToTask()
    {
        ...
        $this->response->redirect($this->helper->url->to('ContactsController', 'task', array('plugin' => 'contacts', 'project_id' => $project['id'], 'task_id' => $task_id)));
    }
