...
<?php else: ?>
    ...
    <tr>
        <td><?= $this->url->link('<i class="fa  fa-arrow-down" aria-hidden="true"></i>', 'ContactsController', 'removeFromTask', array('contacts_id' => $value['contacts_id'], 'project_id' => $project['id'], 'task_id' => $task['id'], 'plugin' => 'contacts'), false, '') ?></td>
...
<?php else: ?>
    ...
    <?php $values = $this->ContactsHelper->getContactByID($value['contacts_id']) ?>
    <tr>
        <td><?= $this->url->link('<i class="fa  fa-arrow-up" aria-hidden="true"></i>', 'ContactsController', 'addToTask', array('contacts_id' => $value['contacts_id'], 'project_id' => $project['id'], 'task_id' => $task['id'], 'plugin' => 'contacts'), false, '') ?></td>
        <td><?= (empty($values[1]))?"":$values[1]['value'] ?></td>
        <td><?= (empty($values[2]))?"":$values[2]['value'] ?></td>
        <td><?= (empty($values[3]))?"":$values[3]['value'] ?></td>
        <td>
